...
public class JsonCreater {
	...
	public static JSONObject createJsonAPI(JSONObject productDetail, int adults, int minors,int cover, File directory) {
	...
	//	Formatter.write(directory,(+age +" years,"+((gender)?"Male":"Female") + ","+relation + ","+sumInsured+","));
		...
		result = formatjSON(md,productDetail,directory);
		return result;
	}
	
	public static JSONObject formatjSON(MemberData[] md,JSONObject productDetail,File directory) {
	...
	//		Formatter.write(directory,(md[i].age +" years,"+md[i].gender + ","+md[i].relation + ",SI:"+md[i].sumInsured+","));
			...
			/*if(deductibleAmount!=null) {
				baseProduct.put("deductibleAmount", deductibleAmount);
				Formatter.write(directory,("deductible:"+deductibleAmount+","));
			}*/
		...
		Formatter.write(directory, excelEntry);
	...
	public static void main(String s[]) throws Exception {
		Object[]  covers = {"0"};
		String data = new String(Files.readAllBytes(Paths.get("/home/hi.agrawal/product api")));
		Integer maxcover = 500000;
		JSONObject productDetail = new JSONObject(data);
		SortedSet<Integer> st =  ProductDetails.getValidSumInsured(productDetail);
		st.removeIf(suminsured -> (suminsured<maxcover));
		covers = st.toArray();
	       for (Object object : covers) {  
